...
UvVar nrf1 = "";
#endregion
public FL769(ref UvVar TRANSFER_ID, ref UvVar INTERACT, ref UvVar REDISPLAY) {
...
#endregion
/*     EXPORT TRANSFER LIST / SELECTION SCREEN */
PROGRAM = "FL769";
...
FOOTER = " Transfer No, 'Dn' to Delete, 'N'ew or <Return> ";
TRANSFER_LIST = "";
/*  Box heading */
...
INSTRUCTION = "SSELECT FL.UNIVERSE.TRANSFER = \"CONTROL*]\"";
nrf0 = "SO";
...
TRANSFER_ID = TRANSFER_LIST[1, WAIT].Substring(9, 99);
if (TRANSFER_ID == "") {
nrf0 = 1;
nrf1 = INIT;
FL006 = new FL006(ref PROGRAM, ref nrf0, ref nrf1);
...
TRANSFER_ID = TRANSFER_LIST[1, WAIT.Substring(2, 99)];
if (TRANSFER_ID == "") {
nrf0 = 3;
nrf1 = INIT;
FL006 = new FL006(ref PROGRAM, ref nrf0, ref nrf1);
} else {
FOOTER2 = " Delete " + TRANSFER_ID.Substring(9, 99) + "? (Y/N) ";
PRINT(AT(79, 2 + BOTTOM_WINDOW) + NRM_VID);
...
DELETE(FL_UNIVERSE_TRANSFER, TRANSFER_ID);
nrf0 = "D";
nrf1 = "FL.UNIVERSE.TRANSFER";
FLTX2 = new FLTX2(ref nrf0, ref nrf1, ref TRANSFER_ID);
TRANSFER_LIST = DELETE(TRANSFER_LIST, 1, WAIT.Substring(2, 99), 0);
}
TRANSFER_NO = 1;
INTERACT = LIMIT;
}
...
do {
CNT = CNT + 1;
TRANSFER_ID = TRANSFER_LIST[1, TRANSFER_NO];
if (CNT >= WINDOW_DEPTH) break;
PRINT(AT(79, CNT + 10) + NRM_VID);
if (TRANSFER_ID == "") {
PRINT(AT(LEFT_WINDOW, CNT + 10) + COLOR_C + FORMAT("", WINDOW_JUST));
} else {
if (!MATREAD(ref TRANS_REC, FL_UNIVERSE_TRANSFER, TRANSFER_ID)) { MAT(ref TRANS_REC, ""); }
DISPLAY_LINE = FORMAT(TRANSFER_NO, "R#4") + "  " + FORMAT(TRANSFER_ID.Substring(9, 99), "L#15");
...
/*  'blank' window */
if (TRANSFER_LIST != "") { L1000(); } else { BLANK_DISPLAY_COMPLETE = TRUE; }
