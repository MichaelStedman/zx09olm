...
if (DD_STAGE == 3 && DD_PAYMENT_DATE == DD_LAST_DATE_PARAM[1, DAY_POS]) {
nrf0 = 7;
...
/* *           END */
if (BACS_SPECIAL_ROUTINE.Substring(1, 1) == "*") { BACS_SPECIAL_ROUTINE = ""; }
BANK_KEY = FIELD(DD_KEY, "*", 1) + "*" + FIELD(DD_KEY, "*", 2);
if (READV(ref BACS_REF, COMMON.FILES[FL_BANK_DETAILS], BANK_KEY, 7)) {
// BEGIN CASE
// CASE
if (BACS_REF != "" && BACS_SPECIAL_ROUTINE == "") {
BACS_REF = "1*" + BACS_REF + "*1";
}// CASE
else if (BACS_REF != "" && BACS_SPECIAL_ROUTINE != "") {
...
CallAt_BACS_SPECIAL_ROUTINE = new CallAt_BACS_SPECIAL_ROUTINE(ref BACS_SPECIAL_ROUTINE, ref OUR_REF);
BACS_REF = OUR_REF;
}
// END CASE
}
