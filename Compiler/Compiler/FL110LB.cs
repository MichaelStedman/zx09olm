...
namespace Compiler
{
   public class FL110LB : UvBase
   {
      ...
      public FL110LB()
      {
         ...
         #endregion
         if (!READ(ref PRINT_REC, COMMON.FILES[FL_CONTROL], "PR.ASS")) { PRINT_REC = ""; }
         TILLS = PRINT_REC[1];
         /*  DIMENSION RECORDS * */
      ...
      void L7000()
      {
         ...
         if (LOCATE(MAIN_KEY, TILLS, 1, 0, 1, ref POS, "AL"))
         {
            PRINT_REC[1, POS] = MAIN_KEY;
            PRINT_REC[3, POS] = ASSIGN_CMD;
         }
         else
         {
            PRINT_REC = INSERT(PRINT_REC, 1, POS, 0, MAIN_KEY);
            PRINT_REC = INSERT(PRINT_REC, 3, POS, 0, ASSIGN_CMD);
         }
         WRITE(PRINT_REC, COMMON.FILES[FL_CONTROL], "PR.ASS");
         PRINT(AT(10, 13) + "A test print is being sent to the printer you have ");
         PRINT(AT(10, 14) + "specified, if this does not print you need to alter");
         PRINT(AT(10, 15) + "one or other of the above parameters and try again ");
         ...
         {
            PRINTCR(AT(10, 17) + "These paramaters do NOT recognise either the MACHINE or");
            PRINTCR(AT(10, 18) + "the PRINTER names, fix and try again!");
         }
         INPUT(out WAIT, 1);
         return;
         /*   END OF PROGRAM               * */
