...
namespace Compiler
{
   public class FL875 : UvBase
   {
      ...
      public FL875()
      {
         ...
         #endregion
         /*     MAINTAIN CONCESSION ROLL-DOWNS */
         PROMPT("");
         PROGRAM = "FL875";
         SCREEN_CODE = PROGRAM;
         /* ******************************** */
         /*   Define Files Used            * */
         /* ******************************** */
         #region INCLUDE FL.FILES FL.CONCESS.ROLLDOWN
         if (!OPEN("FL.CONCESS.ROLLDOWN", out FL_CONCESS_ROLLDOWN)) { STOP("201", "FL.CONCESS.ROLLDOWN"); }
         ...
         do
         {
            ...
            if (INTERACT == VALID_DATA)
            {
               ...
               {
                  MATWRITE(ROLL_REC, FL_CONCESS_ROLLDOWN, MAIN_KEY);
               }
            }
      ...
      void L0050()
      {
         ...
         do
         {
            ...
            else if (ANS == "D")
            {
               ...
               if (REPLY == "Y" || REPLY == "y")
               {
                  DELETE(FL_CONCESS_ROLLDOWN, MAIN_KEY);
                  INTERACT = EXIT;
               }
               else
               {
      ...
      void L0300()
      {
         ...
         for (I = 1; I <= MAX_FIELDS; I += 1)
         {
            ...
            if (DISP_ATT != 0)
            {
               ...
               if (DISP_CONVERSION != "")
               {
                  PRINT(FORMAT(OCONV(ROLL_REC[DISP_ATT][1, DISP_MV], DISP_CONVERSION), DISP_JUST));
               }
               else
               {
                  PRINT(FORMAT(ROLL_REC[DISP_ATT][1, DISP_MV], DISP_JUST));
               }
            }
         }
         return;
      ...
      void L1000()
      {
         /*   CONCESSION Validation Routine */
         if (ANS == COMMON.HELP)
         {
            FL876 = new FL876(ref ANS, ref INTERACT, ref REDISPLAY);
         ...
         if (ANS != "" && INTERACT == VALID_DATA)
         {
            ...
            {
               MAIN_KEY = VALUE;
               if (!(MATREADU(ref ROLL_REC, FL_CONCESS_ROLLDOWN, MAIN_KEY)))
               {
                  AMENDING = FALSE;
                  MAT(ref ROLL_REC, INIT);
               }
      ...
      void L1100()
      {
         /*   Rolldown concession Validation Routine */
         INTERACT = VALID_DATA;
         return;
         /* ****************** */
         /*   End of Program * */
