...
namespace Compiler
{
   public class FL_BANK_WIZARD : UvBase
   {
      ...
      UvVar SYSTEM14 = "";
      UvVar KEY = "";
      FL_CLIENT_VERSION FL_CLIENT_VERSION;
      UvVar FWCLIENT_VERSION = "";
      UvVar RESPONSE = "";
      #endregion
      public FL_BANK_WIZARD(ref UvVar SORT_CODE, ref UvVar ACCOUNT_NUMBER, ref UvVar INTERACT)
      {
         ...
         #endregion
         //    VALIDATE BANK DETAILS USING QAS/EIGER BANK WIZARD
         PROMPT("");
         ...
         if (SORT_CODE != "" && ACCOUNT_NUMBER != "")
         {
            ...
            //    Get Version of FWTerm
            FL_CLIENT_VERSION = new FL_CLIENT_VERSION(ref FWCLIENT_VERSION);
            //  Instruct FWTERM to fire up Bank Wizard
            if (FWCLIENT_VERSION >= 4.15M && CLIENT_APP_TYPE.Substring(1, 1) != "W")
            {
               ECHO_OFF();
               PRINT(BANKVALIDATE + SORT_CODE + "," + ACCOUNT_NUMBER + EOC);
               INPUT(out RESPONSE);
               // BEGIN CASE
               // CASE
               if (RESPONSE == -1)
               {
                  //              BANK WIZARD NOT AVAILABLE
               }// CASE
               else if (RESPONSE == 0)
               {
                  //              VALIDATION FAILED
