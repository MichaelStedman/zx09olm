...
#endregion
/*     CONTROLLING ROUTINE FOR COURSE ENROLMENT */
PROMPT("");
PROGRAM = "FL716";
SCREEN_CODE = PROGRAM;
INTERACT = "";
...
FL715 = new FL715(ref STUDENT_AND_APACS_NOS, ref COURSE_MSTR_REC, ref COURSE_DET_REC, ref ENROL_REC, ref STUDENT_REC, ref TICKET_REC, ref RECEIPT_REQUIRED, ref NEW_COURSE, ref COURSE_BALANCE, ref PAYMENT_MVC, ref GROUP_MULTIVALUE, ref OLD_COURSE_DATE, ref WAITING_LIST_ENROLMENT, ref WAIT_MAST_REC, ref BONUS_REDEEM_REC, ref PROMOTION, ref MOP);
...
NARR_MV = NARR_MV + 1;
if (PAYMENT_MVC != "") {
PAYMENT_AMOUNT = -STUDENT_REC[STUDENT_TRANS_AMOUNTS][1, PAYMENT_MVC];
} else {
PAYMENT_AMOUNT = 0;
}
NARRATIVE[1, NARR_MV] = "Fee = " + OCONV(ENROL_REC[ENROL_PRICE], "MD2") + " Paid " + OCONV(PAYMENT_AMOUNT, "MD2") + " o/s " + OCONV(COURSE_BALANCE, "MD2");
TICK_REC = "";
/*  Dynamic equivalent */
TICK_ID = "";
PRINT_TICKET_NO = "";
...
/* !*           IF TICK.TOTAL > 0 AND CURRENT.PROMOTIONS # "" AND STUDENT.IS.MEMBER THEN */
/*  Only award points when the full balance has been settled */
if (COURSE_BALANCE == 0 && CURRENT_PROMOTIONS != "" && STUDENT_IS_MEMBER) {
PERIOD = 1;
...
nrf1 = TICKET_REC[TICK_TOTAL];
nrf2 = 1;
nrf3 = ENROL_REC[ENROL_CONCESSION][1, 1];
FL285 = new FL285(ref TICK_REC, ref BONUS_REDEEM_REC, ref nrf0, ref nrf1, ref nrf2, ref nrf3, ref PERIOD, ref STUDENT_ID);
...
TICKET_REC[TICK_REDEEMED] = ENROL_REC[ENROL_TOTAL_REDEMPTION];
TICKET_REC[TICK_POINTS_REDEEMED] = ENROL_REC[ENROL_TOTAL_POINTS];
...
/*  DJA 2.5.01 */
if (PROG_MESS[16] != "") {
/*  New student no was generated/input */
WAIT_STUDENT_ID = PROG_MESS[16];
} else {
WAIT_STUDENT_ID = STUDENT_ID;
}
/* !* MATREADU WAIT.MAST.REC FROM FL.WAIT.MASTER, COURSE.MSTR.ACTIVITY.CODE ELSE MAT WAIT.MAST.REC = "" */
if (LOCATE(WAIT_STUDENT_ID, WAIT_MAST_REC[WAIT_MAST_IDS], 1, 0, 1, ref POS, "AL")) {
...
if (MATREADU(ref WAIT_STUDENT_REC, FL_WAIT_STUDENTS, WAIT_STUDENT_ID)) {
...
DELETE(FL_WAIT_STUDENTS, WAIT_STUDENT_ID);
} else {
MATWRITE(WAIT_STUDENT_REC, FL_WAIT_STUDENTS, WAIT_STUDENT_ID);
}
/*  29.8.01 DJA */
} else {
RELEASE();
