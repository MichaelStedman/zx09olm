...
UvVar nrf1 = "";
#endregion
public FL719(ref UvVar INSTRUCTOR_ID, ref UvVar INTERACT, ref UvVar REDISPLAY) {
...
L2200();
PRINT(AT(ST_COL, ST_ROW) + COLOR_C);
...
PREFIX = " WITH";
if (NAME_REC[SURNAME] != "") {
SELECTION_CRITERIA = PREFIX + " SURNAME = \"[" + NAME_REC[SURNAME] + "]\"";
PREFIX = "";
}
}
return;
...
PRINT(AT(2, CNT + 10) + COLOR_C + SPACE(76));
...
COURSE_LINE = FORMAT(INSTRUCTOR_NO, "R#4") + "  " + FORMAT(TRIM(INSTRUCTOR_REC[INSTRUCTOR_TITLE] + " " + INSTRUCTOR_REC[INSTRUCTOR_FORENAME] + " " + INSTRUCTOR_REC[INSTRUCTOR_SURNAME]), "L#23") + "  " + FORMAT(QUALIFICATIONS, "L#37");
PRINT(AT(LEFT_WINDOW - 1, CNT + 10) + COLOR_C);
