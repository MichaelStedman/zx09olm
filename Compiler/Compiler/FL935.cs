...
UvVar nrf1 = "";
#endregion
public FL935(ref UvVar AREA_ID, ref UvVar INTERACT, ref UvVar REDISPLAY, ref UvVar SPARE1) {
...
CODES_FOUND = FALSE;
INSTRUCTION = "SSELECT FL.FLOOR.AREAS # \"[MASTER]\" AND WITH CENTRE = \"" + COMMON.CENTRE + "\"";
nrf0 = "SO";
...
do {
CNT = CNT + 1;
AREA_ID = RESOURCE_LIST[1, AREA_NO];
...
if (!MATREAD(ref FLOORS_REC, COMMON.FILES[FL_FLOOR_AREAS], AREA_ID)) { MAT(ref FLOORS_REC, ""); }
DISPLAY_LINE = FORMAT(AREA_NO, "R#4") + "  " + FORMAT(FIELD(AREA_ID, "*", 2), "L#6") + " " + FORMAT(FLOORS_REC[FLOOR_DESC], "L#30");
