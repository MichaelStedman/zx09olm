...
UvVar nrf1 = "";
#endregion
public FL819(ref UvVar ACTIVITY_ID, ref UvVar INTERACT, ref UvVar REDISPLAY, ref UvVar SELECTED_CENTRE, ref UvVar ACCOUNT_NAME, ref UvVar RESOURCES_ONLY) {
...
FOOTER = " Select Act. No. or <return> to scroll ";
ACTIVITY_LIST = "";
/*  Box heading */
...
INSTRUCTION = "SSELECT " + SPORTS_FILE + " BY 1";
} else {
INSTRUCTION = "SSELECT " + SPORTS_FILE + " WITH 16 # \"C\" BY 1";
}
nrf0 = "SO";
...
ACTIVITY_ID = ACTIVITY_LIST[1, WAIT];
if (ACTIVITY_ID == "") {
nrf0 = 1;
nrf1 = INIT;
FL006 = new FL006(ref PROGRAM, ref nrf0, ref nrf1);
...
do {
CNT = CNT + 1;
ACTIVITY_ID = ACTIVITY_LIST[1, ACTIVITY_NO];
if (CNT >= WINDOW_DEPTH) break;
PRINT(AT(79, CNT + 10) + NRM_VID);
if (ACTIVITY_ID == "") {
PRINT(AT(LEFT_WINDOW, CNT + 10) + DIM_VID + FORMAT("", WINDOW_JUST));
} else {
if (!READV(ref SPORT_DESC, FL_SPORTS, ACTIVITY_ID, 1)) { SPORT_DESC = ""; }
DISPLAY_LINE = FORMAT(ACTIVITY_NO, "R#4") + "  " + FORMAT(ACTIVITY_ID, "L#7") + "  " + SPORT_DESC;
PRINT(AT(LEFT_WINDOW, CNT + 10) + DIM_VID + FORMAT(DISPLAY_LINE, WINDOW_JUST));
}
ACTIVITY_NO = ACTIVITY_NO + 1;
} while (true);
/*  Don't prompt for course no. if this is the initial call to display a */
/*  'blank' window */
if (ACTIVITY_LIST != "") { L1000(); } else { BLANK_DISPLAY_COMPLETE = TRUE; }
