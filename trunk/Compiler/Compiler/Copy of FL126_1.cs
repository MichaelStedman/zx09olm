...
UvVar nrf1 = "";
#endregion
public FL126_1(ref UvVar REFERENCE, ref UvVar TYPE, ref UvVar DEBIT_NUMBER, ref UvVar DEBIT_LIST, ref UvVar DD_DAY) {
...
if (!(READ(ref WINDOW, FL_CONTROL, WINDOW_ID))) {
WINDOW = 9;
WINDOW[2] = 18;
WINDOW[3] = 2;
WINDOW[4] = 77;
WINDOW[5] = "      Reference   Account Name              Sort Cd Acct. No  Amount   Rjctd";
...
/*      PRINT @(LEFT.WINDOW+2+LEN(FOOTER),1+BOTTOM.WINDOW): */
/*  Return to calling program */
return;
}
void L1000() {
/*  GATHER DEBIT NUMBERS */
if (!(READV(ref DDREC_REQUIRED, FL_CONTROL, "DDREC.REQD", 1))) {
DDREC_REQUIRED = TRUE;
}
DEBIT_NUMBER = 0;
DEBIT_CNT = 0;
...
if (MATREAD(ref DD_WORK_REC, FL_DD_WORK, REQUIRED_ID)) {
DEBIT_CNT = DEBIT_CNT + 1;
DEBIT_LIST[1, -1] = DEBIT_SET;
DEBIT_TEXT_LIST[1, -1] = FORMAT(REQUIRED_ID, "L#12") + " " + FORMAT(DD_WORK_REC[DD_WORK_ACCT_NAME], "L#25") + " " + FORMAT(DD_WORK_REC[DD_WORK_SORT_CODE], "L#6") + " " + FORMAT(DD_WORK_REC[DD_WORK_ACCT_NO], "L#10") + FORMAT(OCONV(DD_WORK_REC[DD_WORK_AMOUNT], COMMON.MD2), "R#8") + " " + DD_WORK_REC[DD_WORK_REJECTED];
}
...
DEBIT_LINE = FORMAT(DEBIT_CNT, "R#4") + SPACE(2) + FORMAT(DEBIT_TEXT, "L#70");
PRINT(AT(LEFT_WINDOW - 1, CNT + TOP_WINDOW) + COLOR_C);
