...
namespace Compiler
{
   public class FL751 : UvBase
   {
      ...
      public FL751()
      {
         ...
         #endregion
         /*     REPORT OF COURSE STUDENTS WITH DEBIT/CREDIT BALANCES - METRO SPECIAL */
         /*     EXACT COPY OF FL743 BUT INITIAL SELECTS LOOK FOR STUDENTS NOS */
         /*     STARTING 'SS'                         = */
         PROGRAM = "FL751";
         /* ************************ */
         /*     DEFINE FILES USED  * */
      ...
      L3: ;
         ...
         INSTRUCTION = "SSELECT FL.STUDENTS = \"SS]\" WITH BALANCE > \"0.00\" BY SURNAME";
         nrf0 = "SO";
         ...
         do
         {
            ...
            do
            {
               ...
               if (LOCATE(DET_ID, DET_ID_LIST, 1, 0, 1, ref DET_POS, "AL"))
               {
                  ...
                  BALANCE_TOTAL = BALANCE_TOTAL + STUDENT_REC[STUDENT_TRANS_AMOUNTS][1, TRANS_NO];
               }
               else
               {
            ...
            ULINE_PRINTED = FALSE;
            TOTAL_PRINTED = FALSE;
            if (INST_BALANCE_TOTAL >= MINIMUM_ARREARS)
            {
               ...
               PRINT(SPACE(1));
               PRINT(FORMAT(COURSE_LIST[1, 1], "L#10"));
               PRINT(SPACE(1));
      ...
      void L0175()
      {
         /*  Print totals                             * */
         /* *********************************************** */
         PRINT(FORMAT(OCONV(STUDENT_REC[STUDENT_BALANCE], "MD2"), "R#8"));
         PRINT(FORMAT(OCONV(INST_BALANCE_TOTAL, "MD2"), "R#10"));
         L1();
         TOTAL_PRINTED = TRUE;
         GRAND_TOTAL_BALANCE = GRAND_TOTAL_BALANCE + STUDENT_REC[STUDENT_BALANCE];
