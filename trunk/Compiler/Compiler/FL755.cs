...
UvVar TOGGLE_ROW = "";
#endregion
public FL755(ref UvVar ACTION, ref UvVar TOGGLE_POS, ref UvVar STUDENT_SELECT_LIST, ref UvVar COURSE_DET_STUDENT_IDS, ref UvVar REDISPLAY, ref UvVar STUDENT_COUNT, ref UvVar STUDENT_LIST, ref UvVar SURNAME_LIST, ref UvVar FULL_NAME_LIST, ref UvVar CONCESS_LIST, ref UvVar BALANCE_LIST, ref UvVar ADJUSTMENT_LIST, ref UvVar INSTALMENT_LIST, ref UvVar ERROR_LIST, ref UvVar XFER_LIST, ref UvVar WINDOW_DEPTH) {
this.ACTION = ACTION;
...
this.INSTALMENT_LIST = INSTALMENT_LIST;
this.ERROR_LIST = ERROR_LIST;
this.XFER_LIST = XFER_LIST;
...
#endregion
/*     DISPLAY COURSE MEMBER DETAILS FOR MANUAL RE-ENROLMENT */
PROGRAM = "FL755";
INTERACT = INVALID_DATA;
REDISPLAY = "";
/* *IF ACTION # 1 THEN CRT @(0,0):EOL:"ACTION=":ACTION:"  TOGGLE.POS=":TOGGLE.POS:"  SURNAME.LIST=":SURNAME.LIST:"  XFER.LIST=":XFER.LIST:"  STUDENT.COUNT=":STUDENT.COUNT:EOL:;INPUT FLEX,2: */
...
NAME = TRIM(MEMBERS_REC[MEMBER_SURNAMES][1, 1] + ", " + MEMBERS_REC[MEMBER_TITLES][1, 1] + " " + MEMBERS_REC[MEMBER_FORENAMES][1, 1].Substring(1, 1));
} else {
...
if (STUDENT_REC[STUDENT_TRANS_TYPES][1, TRANS_NO] == "A") {
...
XFER_LIST = INSERT(XFER_LIST, 1, POS, 0, "");
...
CONCESS_LIST = INSERT(CONCESS_LIST, 1, POS, 0, ENROL_REC[ENROL_CONCESSION][1, 1]);
if (DCOUNT(ENROL_REC[ENROL_INST_DATES], VM) >= 1) {
INSTALMENT_LIST = INSERT(INSTALMENT_LIST, 1, POS, 0, "Yes");
} else {
INSTALMENT_LIST = INSERT(INSTALMENT_LIST, 1, POS, 0, "No");
}
}
}
COURSE_DETAIL_ID = PROG_MESS[3] + "*" + PROG_MESS[2];
...
COURSE_LINE = COURSE_LINE + FORMAT(FULL_NAME_LIST[1, STUDENT_COUNT], "L#25") + SPACE(2) + FORMAT(CONCESS_LIST[1, STUDENT_COUNT], "L#10") + SPACE(2) + FORMAT(OCONV(BALANCE_LIST[1, STUDENT_COUNT], "MD2"), "R#6") + FORMAT(INSTALMENT_LIST[1, STUDENT_COUNT], "R#9") + FORMAT(ERROR_LIST[1, STUDENT_COUNT], "R#4");
// BEGIN CASE
// CASE
if (XFER_LIST[1, STUDENT_COUNT] == TRUE) {
PRINT(AT(78, CNT + TOP_WINDOW + 1) + NRM_VID);
PRINT(AT(LEFT_WINDOW - 1, CNT + TOP_WINDOW + 1) + DIM_REV_UND_VID);
}// CASE
else if (STUDENT_SELECT_LIST[1, STUDENT_COUNT] == TRUE) {
PRINT(AT(78, CNT + TOP_WINDOW + 1) + NRM_VID);
...
COURSE_LINE = COURSE_LINE + FORMAT(FULL_NAME_LIST[1, TOGGLE_POS], "L#25") + SPACE(2) + FORMAT(CONCESS_LIST[1, TOGGLE_POS], "L#10") + SPACE(2) + FORMAT(OCONV(BALANCE_LIST[1, TOGGLE_POS], "MD2"), "R#6") + FORMAT(INSTALMENT_LIST[1, TOGGLE_POS], "R#9") + FORMAT(ERROR_LIST[1, TOGGLE_POS], "R#4");
// BEGIN CASE
// CASE
if (XFER_LIST[1, TOGGLE_POS] == TRUE) {
PRINT(AT(78, TOGGLE_ROW) + NRM_VID);
PRINT(AT(LEFT_WINDOW - 1, TOGGLE_ROW) + DIM_REV_UND_VID);
}// CASE
else if (STUDENT_SELECT_LIST[1, TOGGLE_POS] == TRUE) {
PRINT(AT(78, TOGGLE_ROW) + NRM_VID);
