...
namespace Compiler
{
public class FWSHOWMESSAGEDIALOG : UvBase
{
...
UvVar WINDOWFORM = "";
#endregion
public FWSHOWMESSAGEDIALOG(ref UvVar MESSAGE, ref UvVar STYLE, ref UvVar BUTTONS, ref UvVar BUTTONSIZE, ref UvVar VALUE) {
...
PROMPT("");
ECHO_OFF();
PRINT(SHOWMESSAGEDIALOG + MESSAGE + "," + STYLE + "," + BUTTONS + "," + BUTTONSIZE + EOC);
do {
INPUT(out MESSAGE);
if (DEBUG_VERSION) { PRINT(CHAR(18) + "SMD=" + MESSAGE + CRLF + CHAR(20)); }
if (INDEX(MESSAGE[1, 1], "dialog", 1)) break;
MESSAGE_QUEUE[-1] = MESSAGE;
} while (true);
VALUE = DELETE(MESSAGE, 1, 1, 0);
ECHO_ON();
return;
}
}
