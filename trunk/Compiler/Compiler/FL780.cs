...
UvVar nrf1 = "";
#endregion
public FL780(ref UvVar AWARD_ID, ref UvVar INTERACT, ref UvVar REDISPLAY) {
...
#endregion
/*     AWARD CODE LIST / SELECTION SCREEN */
PROGRAM = "FL780";
...
FOOTER = " Select Award No. or <Return> to scroll ";
AWARD_LIST = "";
/*  Box heading */
...
INSTRUCTION = "SSELECT FL.COURSE.AWARDS BY 1";
nrf0 = "SO";
...
do {
CNT = CNT + 1;
AWARD_ID = AWARD_LIST[1, AWARD_NO];
if (CNT >= WINDOW_DEPTH) break;
PRINT(AT(79, CNT + 10) + NRM_VID);
if (AWARD_ID == "") {
PRINT(AT(LEFT_WINDOW, CNT + 10) + COLOR_C + FORMAT("", WINDOW_JUST));
} else {
if (!MATREAD(ref AWARD_REC, FL_COURSE_AWARDS, AWARD_ID)) { MAT(ref AWARD_REC, ""); }
DISPLAY_LINE = FORMAT(AWARD_NO, "R#4") + "  " + FORMAT(AWARD_ID, "L#7") + "  " + AWARD_REC[AWARD_DESCRIPTION];
...
/*  'blank' window */
if (AWARD_LIST != "") { L1000(); } else { BLANK_DISPLAY_COMPLETE = TRUE; }
