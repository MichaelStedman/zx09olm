...
namespace Compiler
{
   public class FL874 : UvBase
   {
      ...
      public FL874()
      {
         ...
         #endregion
         /*     MAINTAIN GENERAL WEB PARAMETERS 2 */
         PROMPT("");
         PROGRAM = "FL874";
         SCREEN_CODE = PROGRAM;
      ...
      void L1200()
      {
         /*  Restrict re-enrolment to current course place */
         if (ANS == "" || ANS == "Y" || ANS == "N")
         {
            INTERACT = VALID_DATA;
         }
      ...
      void L1300()
      {
         /*  Mandatory payments for members to re-enrol */
         if (ANS == "" || ANS == "Y" || ANS == "N")
         {
            INTERACT = VALID_DATA;
         }
      ...
      void L1400()
      {
         /*  Validate course balances at login */
         if (ANS == "" || ANS == "Y" || ANS == "N")
         {
            INTERACT = VALID_DATA;
         }
      ...
      void L1600()
      {
         /*  Automatic bonus redemption to reduce web prices */
         if (ANS == "" || ANS == "Y" || ANS == "N")
         {
            INTERACT = VALID_DATA;
         }
      ...
      void L1700()
      {
         /*  Single login per member for eBookings? */
         if (ANS == "" || ANS == "Y" || ANS == "N")
         {
            INTERACT = VALID_DATA;
         }
      ...
      void L1800()
      {
         /*  Max bookings on the bookahead day (for MEMBERS) */
      ...
      void L1900()
      {
         /*  Membership type for registered users */
         INTERACT = VALID_DATA;
         return;
      }
      void L2000()
      {
         /*  Centre code for registered users */
         INTERACT = VALID_DATA;
         return;
      }
      void L2100()
      {
         /*  Validation mask for GL codes */
         INTERACT = VALID_DATA;
         return;
      }
      void L2200()
      {
         /*  Maximum timetable columns per web page */
         if (ANS == "" || MATCH(ANS, "1N") || MATCH(ANS, "2N"))
         {
            INTERACT = VALID_DATA;
         }
      ...
      void L2300()
      {
         /*  Max future bookings (internal AND external) */
         if (ANS == "" || MATCH(ANS, "1N") || MATCH(ANS, "2N"))
         {
            INTERACT = VALID_DATA;
         }
      ...
      void L2500()
      {
         /*  Minimum timeslot interval for booking sheet */
      ...
      void L8000()
      {
         ...
         TEMP_REC[1] = FW_WEB_CONTROL[19];
         if (TEMP_REC[1].Substring(1, 1) == "*") { TEMP_REC[1] = ""; }
         TEMP_REC[2] = FW_WEB_CONTROL[20];
         if (TEMP_REC[2].Substring(1, 1) == "*") { TEMP_REC[2] = ""; }
         TEMP_REC[3] = FW_WEB_CONTROL[21];
         if (TEMP_REC[3].Substring(1, 1) == "*") { TEMP_REC[3] = ""; }
         TEMP_REC[4] = FW_WEB_CONTROL[22];
         if (TEMP_REC[4].Substring(1, 1) == "*") { TEMP_REC[4] = ""; }
         TEMP_REC[5] = FW_WEB_CONTROL[23];
         if (TEMP_REC[5].Substring(1, 1) == "*") { TEMP_REC[5] = ""; }
         TEMP_REC[6] = FW_WEB_CONTROL[24];
         if (TEMP_REC[6].Substring(1, 1) == "*") { TEMP_REC[6] = ""; }
         TEMP_REC[7] = FW_WEB_CONTROL[25];
         if (TEMP_REC[7].Substring(1, 1) == "*") { TEMP_REC[7] = ""; }
         TEMP_REC[8] = FW_WEB_CONTROL[26];
         if (TEMP_REC[8].Substring(1, 1) == "*") { TEMP_REC[8] = ""; }
         TEMP_REC[9] = FW_WEB_CONTROL[27];
         if (TEMP_REC[9].Substring(1, 1) == "*") { TEMP_REC[9] = ""; }
         TEMP_REC[10] = FW_WEB_CONTROL[28];
         if (TEMP_REC[10].Substring(1, 1) == "*") { TEMP_REC[10] = ""; }
         TEMP_REC[11] = FW_WEB_CONTROL[29];
         if (TEMP_REC[11].Substring(1, 1) == "*") { TEMP_REC[11] = ""; }
         TEMP_REC[12] = FW_WEB_CONTROL[30];
         if (TEMP_REC[12].Substring(1, 1) == "*") { TEMP_REC[12] = ""; }
         TEMP_REC[13] = FW_WEB_CONTROL[31];
         if (TEMP_REC[13].Substring(1, 1) == "*") { TEMP_REC[13] = ""; }
         TEMP_REC[14] = FW_WEB_CONTROL[32];
         if (TEMP_REC[14].Substring(1, 1) == "*") { TEMP_REC[14] = ""; }
         TEMP_REC[15] = FW_WEB_CONTROL[33];
         if (TEMP_REC[15].Substring(1, 1) == "*") { TEMP_REC[15] = ""; }
         TEMP_REC[16] = FW_WEB_CONTROL[34];
         if (TEMP_REC[16].Substring(1, 1) == "*") { TEMP_REC[16] = ""; }
         TEMP_REC[17] = FW_WEB_CONTROL[35];
      ...
      void L9000()
      {
         /*  UNLOAD TEMP.REC AND WRITE DATA    * */
         /* **************************************** */
         FW_WEB_CONTROL[19] = TEMP_REC[1];
         FW_WEB_CONTROL[20] = TEMP_REC[2];
         FW_WEB_CONTROL[21] = TEMP_REC[3];
         FW_WEB_CONTROL[22] = TEMP_REC[4];
         FW_WEB_CONTROL[23] = TEMP_REC[5];
         FW_WEB_CONTROL[24] = TEMP_REC[6];
         FW_WEB_CONTROL[25] = TEMP_REC[7];
         FW_WEB_CONTROL[26] = TEMP_REC[8];
         FW_WEB_CONTROL[27] = TEMP_REC[9];
         FW_WEB_CONTROL[28] = TEMP_REC[10];
         FW_WEB_CONTROL[29] = TEMP_REC[11];
         FW_WEB_CONTROL[30] = TEMP_REC[12];
         FW_WEB_CONTROL[31] = TEMP_REC[13];
         FW_WEB_CONTROL[32] = TEMP_REC[14];
         FW_WEB_CONTROL[33] = TEMP_REC[15];
         FW_WEB_CONTROL[34] = TEMP_REC[16];
         FW_WEB_CONTROL[35] = TEMP_REC[17];
