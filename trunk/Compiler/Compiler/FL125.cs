...
#endregion
/*  Prepare Direct Debit List (Creates FL.DD.WORK) */
...
if (PRINT_BANK_NAME) {
/*            INSTR = "SORT ":FILE.NAME:" # 'CONTROL.TOTALS' MEMBER.NO BACS.REF MEMBER.TYPE 1 2 4 5 7 TOTAL 3 ID-SUPP HEADING " */
INSTR = "SORT " + FILE_NAME + " # 'CONTROL.TOTALS' MEMBER.NO 1 2 4 5 TOTAL 3 BACS.REF MEMBER.TYPE 7 ID-SUPP HEADING ";
} else {
/*            INSTR = "SORT ":FILE.NAME:" # 'CONTROL.TOTALS' MEMBER.NO BACS.REF MEMBER.TYPE 1 2 5 7 TOTAL 3 ID-SUPP HEADING " */
INSTR = "SORT " + FILE_NAME + " # 'CONTROL.TOTALS' MEMBER.NO 1 2 5 TOTAL 3 BACS.REF MEMBER.TYPE 7 ID-SUPP HEADING ";
}
INSTR = INSTR + ZZZ_QUOTE + CENTRENAME[1] + " - BACS Direct Debit Submission Report for " + OCONV(DD_PAYMENT_DATE, COMMON.D2) + " 'LT'";
INSTR = INSTR + ZZZ_QUOTE + " GRAND-TOTAL " + ZZZ_QUOTE + "Total Amount'U'";
...
else if (DD_PROCESS_DATE < TODAYS_DATE) {
nrf0 = 1;
nrf1 = "";
FL006 = new FL006(ref PROGRAM, ref nrf0, ref nrf1);
}// CASE
...
if (DD_PAYMENT_DATE - CONTROLS_REC[DD_LAST_DATE_PARAM][1, DAY_POS] > 31) {
nrf0 = 6;
nrf1 = "";
FL006 = new FL006(ref PROGRAM, ref nrf0, ref nrf1);
...
if (DD_STAGE == 3 && DD_PAYMENT_DATE == CONTROLS_REC[DD_LAST_DATE_PARAM][1, DAY_POS]) {
nrf0 = 7;
nrf1 = " this run has already been posted !";
FL006 = new FL006(ref PROGRAM, ref nrf0, ref nrf1);
VALID_INPUT = FALSE;
}
} else {
...
OUR_REF = DD_KEY;
/* *           BANK.KEY = FIELD(DD.KEY,"*",1):"*":FIELD(DD.KEY,"*",2) */
/* *           READV BACS.REF FROM FL.BANK.DETAILS,BANK.KEY,7 ELSE */
/* *              BACS.REF = "" */
/* *           END */
/* *           IF BACS.REF # "" THEN */
/* *              BACS.REF = "1*":BACS.REF:"*1" */
/* *           END ELSE */
/* *              BACS.REF = OUR.REF */
/* *           END */
/* *           IF BACS.SPECIAL.ROUTINE # "" AND BACS.SPECIAL.ROUTINE[1,1] # "*" THEN */
/* *              CALL @BACS.SPECIAL.ROUTINE(BACS.REF) */
/* *           END */
if (CONTROLS_REC[BACS_SPECIAL_ROUTINE].Substring(1, 1) == "*") { CONTROLS_REC[BACS_SPECIAL_ROUTINE] = ""; }
BANK_KEY = FIELD(DD_KEY, "*", 1) + "*" + FIELD(DD_KEY, "*", 2);
if (READV(ref BACS_REF, COMMON.FILES[FL_BANK_DETAILS], BANK_KEY, 7)) {
// BEGIN CASE
// CASE
if (BACS_REF != "" && CONTROLS_REC[BACS_SPECIAL_ROUTINE] == "") {
BACS_REF = "1*" + BACS_REF + "*1";
}// CASE
else if (BACS_REF != "" && CONTROLS_REC[BACS_SPECIAL_ROUTINE] != "") {
...
CallAt_CONTROLS_REC = new CallAt_CONTROLS_REC(ref CONTROLS_REC[BACS_SPECIAL_ROUTINE], ref OUR_REF);
BACS_REF = OUR_REF;
}
// END CASE
}
...
/*  add DD Type to Work Rec so that the posting will know what type */
...
if (FIRST_DD_TYPE == "") { FIRST_DD_TYPE = MEMBERS_REC[MEMBER_TYPE][1, 1]; }
